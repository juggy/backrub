...
Function.prototype.depends = function(onHash) {
  this._dependsOn = onHash;
  return this;
};
Backbone.Dependable = function(base) {
  var key, setupEvent, val;
  if (!base.trigger && !base.bind) {
    throw "Not a Backbone.Event object";
  }
  setupEvent = function(attr, events) {
    var event, object, path, _results;
    _results = [];
    for (event in events) {
      path = events[event];
      object = Template._getPath(path, base);
      _results.push(object != null ? object.bind(event, function() {
