(function() {
  ...
  Handlebars.JavaScriptCompiler.prototype.nameLookup = function(parent, name, type) {
    if (type === 'context') {
      return "(context.model.get(\"" + name + "\") ? context.model.get(\"" + name + "\") : context." + name + ");";
    } else {
  ...
  _.extend(Backbone.Template.prototype, {
    ...
    makeAlive: function() {
      var query, self;
      query = [];
      _.each(this._createdViews, function(v, id) {
        return query.push("[data-bvid='" + id + "']");
      });
      self = this;
      return $(query.join(",")).each(function() {
        var el, view;
        el = $(this);
  ...
  Handlebars.registerHelper("view", function(viewName, context) {
    ...
    v.render = function() {
      var rendered;
      rendered = this.span(context(this, null, null, context.data));
